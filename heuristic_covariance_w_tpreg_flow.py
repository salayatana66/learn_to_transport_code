...
a covariance flow regularized by the transport
cost
...
from prob_dist_meas import means_loss as means_loss_fn
from prob_dist_meas import covariance_loss as covariance_loss_fn
...
for iteration, data_dict in enumerate(dataloader_train):
    ...
    if (iteration + 1) % n_stats_to_tensorboard == 0:
        crayon_ship_metrics(ccexp, mets, ["means_loss", "covariance_loss", "l2_loss", "total_loss"],
                            iteration)
