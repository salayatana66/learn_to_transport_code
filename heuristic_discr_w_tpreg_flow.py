...
a discrepancy flow regularized by the
transport cost
...
for iteration, data_dict in enumerate(dataloader_train):
    ...
    torch_losses["total_loss"] = torch_losses["l2_loss"] + \
                                 torch_losses["mean_discrepancy"]

    torch_losses_take_step(loss_dict=torch_losses,
                           optimizer=opt_tm,
                           loss_names=["mean_discrepancy", "l2_loss"]
                           )

    roll_average(loss_dict=torch_losses, mets_dict=mets,
                 metrics=["mean_discrepancy", "l2_loss", "total_loss"],
                 iteration=iteration)
    ...
    if (iteration + 1) % n_stats_to_tensorboard == 0:
        crayon_ship_metrics(ccexp, mets, ["mean_discrepancy", "l2_loss", "total_loss"],
                            iteration)
