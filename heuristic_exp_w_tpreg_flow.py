...
a flow by localized gaussian functions
regularized by the transport cost
...
from prob_dist_meas import exp_discrepancy as exp_discrepancy_fn
...
for iteration, data_dict in enumerate(dataloader_train):
    ...
    idx = np.random.choice(Ztr.shape[0], 50)
    
    torch_losses = {
        "exp_discrepancy": lambda_par * exp_discrepancy_fn(TX, Y, Ztr[idx, :], sig[idx]),
        "l2_loss": l2_loss_fn(TX, X)

    }

    torch_losses["total_loss"] = torch_losses["l2_loss"] + \
                                 torch_losses["exp_discrepancy"]

    torch_losses_take_step(loss_dict=torch_losses,
                           optimizer=opt_tm,
                           loss_names=["exp_discrepancy", "l2_loss"]
    )

    roll_average(loss_dict=torch_losses, mets_dict=mets,
                 metrics=["exp_discrepancy", "l2_loss", "total_loss"],
                 iteration=iteration)

    if (iteration + 1) % n_stats_to_tensorboard == 0:
        crayon_ship_metrics(ccexp, mets, ["exp_discrepancy", "l2_loss", "total_loss"],
                            iteration)
