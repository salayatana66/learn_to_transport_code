...
logger = get_logger("heuristic_symdiscr_w_tpreg_flow")
...
# Experiment Name
exp_name = f"symdiscr_tpreg_ninit_{args.n_init}_ntrain_{args.n_train}_lambda_{args.lambda_par}_cutoff_{args.cutoff_par}_{args.today}"
...
for iteration, data_dict in enumerate(dataloader_train):
    ...
    torch_losses["total_loss"] = torch_losses["l2_loss"] + \
                                 torch_losses["sym_mean_discrepancy"]

    torch_losses_take_step(loss_dict=torch_losses,
                           optimizer=opt_tm,
                           loss_names=["sym_mean_discrepancy", "l2_loss"]
                           )

    roll_average(loss_dict=torch_losses, mets_dict=mets,
                 metrics=["sym_mean_discrepancy", "l2_loss", "total_loss"],
                 iteration=iteration)

    if (iteration + 1) % n_stats_to_tensorboard == 0:
        crayon_ship_metrics(ccexp, mets, ["sym_mean_discrepancy", "l2_loss", "total_loss"],
                            iteration)
